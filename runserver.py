...
# the dictionary into the database.py. Database.py handles all fo the
# sql and returns the response. This is all in reg.py for overviews.
# app route get overviews and get details go into reg.py and the fields
# would come from the query. flask.request.args.get you would call this
# 4 times for the arguments. You call this in details too.

#flask.render_template gives the sql response to the html and you need
# to feed in the data (sql response) from database.py

# cookies is in the specs (if the user clicks back, the database can
# repopulate the table). Search criteria persists across page reloads
# for overviews. And this goes in the reg.py, which is the flask page.

# use the query name arguments from reg.py in html files to call it
#-----------------------------------------------------------------------
#put sql logic in database.py.
# In database.py have 2 functions: getOverviews
